...
def choose_random_index(indices):
    """
    Elige un índice random el 0 o el 1
    """
    random_index = choice(indices)
    return random_index[0], random_index[1]
...
def comenzarIAaleatoria():
    """
    Se inicia el modo de juego IA aleatoria
    """
    tablero()
    siguienteJugador = choice([True, False]) #elige de forma aleatoria quien comienza
    player = 1
    finalizar_Juego = False
    indices = list(product(range(DIMENSIONES), repeat = 2)) #recopila los índices para que la IA elija uno aleatorio
        ...
        for evento in pygame.event.get(): #itera sobre todos los eventos que suceden en la pantalla
            ...
            if siguienteJugador: #turno de jugador 
                ...
                if not finalizar_Juego: # finalizar juego es True, ya no entra
                    random_i, random_j = choose_random_index(indices) #escoge unos índices random de variable índices (la tupla)
                    #random_i, random_j = (0,1)
                    while tablero_juego[random_i][random_j] in [1,2]: #si el indice que ha elegido está ocupado vuelve a escoger otro random
                        random_i, random_j = choose_random_index(indices)
                    marcar_x(random_i, random_j, player) 
                    dibujos()
                    ...
                    player = player % 2 + 1 #cambia de jugador
                    siguienteJugador = True #cambia el turno
